...
namespace Dynamo.Notifier
{
    public class NotificationPoller : IDisposable
    {
        ...
        private readonly BeheerderRepository _beheerderRepository;
        ...
        public bool NieuweBeheerderAangemeld;
        ...
        private void _timer_Tick(object sender, EventArgs e)
        {
            if (IsTijdVoorWebSiteUpdate())
            {
                OnNotify(
                    this,
                    new NotificationPollerEventArgs
                    {
                        UpdateWebSite = true
                    });
            }
            ...
            {
                var huidigDagdeel = DateTime.Now.Hour < 19
                    ? 2
                    : 3;
                ...
                if (!_beheerderRepository.IsBeheerderIngelogdVoorDagDeel(huidigDagdeel))
                {
                    if (OnNotify != null)
                    {
                        OnNotify(
                            ...
                            new NotificationPollerEventArgs
                            {
                                Message = "Er is nog geen beheerder aangemeld voor dit dagdeel!"
                            });
                ...
                if (OnNotify != null)
                {
                    if (aantalNieuweberichten == 1)
                    {
                        OnNotify(
                            ...
                            new NotificationPollerEventArgs
                            {
                                Message = string.Format("Je hebt {0} nieuw bericht.", aantalNieuweberichten)
                            });
                    }
                    else
                    {
                        OnNotify(
                            ...
                            new NotificationPollerEventArgs
                            {
                                Message = string.Format("Je hebt {0} nieuwe berichten.", aantalNieuweberichten)
                            });
                    }
                }
            }
        ...
        {
            var dagdeelId = DateTime.Now.Hour <= 18
                ? 2
                : 3;
                        ...
                        if (OnNotify != null)
                        {
                            OnNotify(
                                ...
                                new NotificationPollerEventArgs
                                {
                                    Message =
                                        string.Format(
                                            "Let op! {0} heeft nog een openstaand bedrag van {1:C}",
                                            item.BandNaam,
                                            item.Bedrag)
                                });
                        }
        ...
        {
            return NieuweBeheerderAangemeld
                || ((DateTime.Now.Hour == 17 || DateTime.Now.Hour == 22) && DateTime.Now.Minute > 30);
        }
