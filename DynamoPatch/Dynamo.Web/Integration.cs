...
namespace Dynamo.Web
{
    ...
    {
        public bool IsBeschikbaar(Planning item)
        {
            if ((item.Boekingen.Count == 0 || item.Boekingen.All(boeking => boeking.DatumAfgezegd != null))
                && !IsGesloten(item.Gesloten, item.Datum, item.OefenruimteId, item.DagdeelId))
            {
                return true;
            }
            return false;
        ...
        public void PushWeekDataToServer(DateTime date)
        {
            ...
            var planning =
                new PlanningRepository().Load(x => x.Datum >= eersteDagVanDeWeek && x.Datum <= laatsteDagVanDeWeek);
            ...
            var weekData = new WeekData
            {
                ...
                ruimtes = new Ruimtes
                {
                    r1 = GetDefaultOefenruimteData(),
                    r2 = GetDefaultOefenruimteData(),
                    r3 = GetDefaultOefenruimteData()
                }
            };
            ...
            foreach (var item in planning.ToList())
            {
                if (!IsBeschikbaar(item))
                {
                    if (item.OefenruimteId == 1)
                    {
                        if (item.DagdeelId == 2)
                        {
                            weekData.ruimtes.r1.m[item.Datum.DagVanDeWeek() - 1] = IsGesloten(
                                item.Gesloten,
                                item.Datum,
                                item.OefenruimteId,
                                item.DagdeelId)
                        ...
                        {
                            weekData.ruimtes.r1.a[item.Datum.DagVanDeWeek() - 1] = IsGesloten(
                                item.Gesloten,
                                item.Datum,
                                item.OefenruimteId,
                                item.DagdeelId)
                    ...
                    else if (item.OefenruimteId == 2)
                    {
                        if (item.DagdeelId == 2)
                        {
                            weekData.ruimtes.r2.m[item.Datum.DagVanDeWeek() - 1] = IsGesloten(
                                item.Gesloten,
                                item.Datum,
                                item.OefenruimteId,
                                item.DagdeelId)
                        ...
                        {
                            weekData.ruimtes.r2.a[item.Datum.DagVanDeWeek() - 1] = IsGesloten(
                                item.Gesloten,
                                item.Datum,
                                item.OefenruimteId,
                                item.DagdeelId)
                    ...
                    else if (item.OefenruimteId == 3)
                    {
                        if (item.DagdeelId == 2)
                        {
                            weekData.ruimtes.r3.m[item.Datum.DagVanDeWeek() - 1] = IsGesloten(
                                item.Gesloten,
                                item.Datum,
                                item.OefenruimteId,
                                item.DagdeelId)
                        ...
                        {
                            weekData.ruimtes.r3.a[item.Datum.DagVanDeWeek() - 1] = IsGesloten(
                                item.Gesloten,
                                item.Datum,
                                item.OefenruimteId,
                                item.DagdeelId)
            ...
            var serializedWeekData = JsonConvert.SerializeObject(
                weekData,
                Formatting.None,
                new JsonSerializerSettings
                {
        ...
        private bool IsGesloten(Gesloten gesloten, DateTime date, int oefenruimte, int dagdeel)
        {
            ...
            if (date.Date < gesloten.DatumVan.Date
                || date.Date > gesloten.DatumTot.GetValueOrDefault(new DateTime(2099, 1, 1)))
            {
                return false;
            }

            if ((oefenruimte == 1 && gesloten.Oefenruimte1)
                || (oefenruimte == 2 && gesloten.Oefenruimte2)
                || (oefenruimte == 3 && gesloten.Oefenruimte3))
            {
                if (dagvandeweek == 1
                    && gesloten.Maandag)
                {
        ...
        private void UploadData(string data, string filename)
        {
            ...
            try
            {
                using (var cl = new FtpClient())
                {
                    ...
                    {
                        UserName = "dynamo-data@oefenruimtedynamo.nl",
                        Password = "yrpSDnlm3U3hEUNwHrBh"
                    };
                    cl.Connect();
